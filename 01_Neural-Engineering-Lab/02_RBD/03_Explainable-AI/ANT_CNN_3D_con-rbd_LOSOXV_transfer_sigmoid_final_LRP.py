...
for sub in range(62):
    ...
    transfer_model.add(Conv3D(filters=16, kernel_size=(3, 3, 2), kernel_initializer='he_normal', input_shape=(67, 67, 14, 1), activation='relu'))
    ...
    transfer_model.load_weights('D:/ANT_3D_CNN_source_transfer/transfer_model_weights/transfer_model_weights_sub_%d.h5' % (sub + 1))

    x_test = np.load('D:/ANT_3D_CNN_source_transfer/transfer_model_testset/transfer_model_x_sub_%d.npy' % (sub + 1))
    y_test = np.load('D:/ANT_3D_CNN_source_transfer/transfer_model_testset/transfer_model_y_sub_%d.npy' % (sub + 1))
    ...
    np.save('D:/ANT_3D_CNN_source_transfer/LRP/LRP_TP_sub_%d.npy' % (sub + 1), TP)
    ...
    if np.size(incorrect_x_test_rbd, axis=0) == 0:
        ...
        # save numpy_array to .mat file
        np.save('D:/ANT_3D_CNN_source_transfer/LRP/LRP_FN_sub_%d.npy' % (sub + 1), FN)

terminate_time = timeit.default_timer()         # 종료 시간 체크
print("%f초 걸렸습니다." % (terminate_time - start_time))     # 딥러닝 돌아가는데 걸린 시간

pre_acc = np.load('D:/ANT_3D_CNN_source_transfer/premodel_accuracy/all_PT_acc.npy')
tl_acc = np.load('D:/ANT_3D_CNN_source_transfer/transfer_model_accuracy/all_TL_acc.npy')
