...
for sub in range(62):
    ...
    x_test = np.load('D:/ANT_3D_CNN_source_transfer2/premodel_testset/premodel_x_sub_%d.npy' % (sub + 1))
    y_test = np.load('D:/ANT_3D_CNN_source_transfer2/premodel_testset/premodel_y_sub_%d.npy' % (sub + 1))
    ...
    correct_x_test = np.reshape(x_test[correct_index, :], (-1, 60, 120, 15, 1))
    correct_index_con = np.array(np.where(correct_y_test[:, 0] == 1))
    correct_index_rbd = np.array(np.where(correct_y_test[:, 1] == 1))
    correct_x_test_con = np.reshape(correct_x_test[correct_index_con, :], (-1, 60, 120, 15, 1))  # 정상인을 맞게 예측한 데이터(TN)
    correct_x_test_rbd = np.reshape(correct_x_test[correct_index_rbd, :], (-1, 60, 120, 15, 1))  # 환자를 맞게 예측한 데이터(TP)
    ...
    incorrect_x_test = np.reshape(x_test[incorrect_index, :, :, :], (-1, 60, 120, 15, 1))
    incorrect_index_con = np.array(np.where(incorrect_y_test[:, 0] == 1))
    incorrect_index_rbd = np.array(np.where(incorrect_y_test[:, 1] == 1))
    incorrect_x_test_con = np.reshape(incorrect_x_test[incorrect_index_con, :, :, :], (-1, 60, 120, 15, 1))  # 정상인을 환자로 예측(FP)
    incorrect_x_test_rbd = np.reshape(incorrect_x_test[incorrect_index_rbd, :, :, :], (-1, 60, 120, 15, 1))  # 환자를 정상인으로 예측(FN)
    ...
    for m in range(np.size(correct_x_test_rbd, axis=0)):
        analysed_results_TP = analyzer.analyze(np.reshape(correct_x_test_rbd[m, :, :, :, :], (-1, 60, 120, 15, 1)))
        TP[m, :, :, :, :] = analysed_results_TP

    # save numpy_array to .mat file
    np.save('D:/ANT_3D_CNN_source_transfer2/LRP/LRP_TP_sub_%d.npy' % (sub + 1), TP)
    ...
    if np.size(incorrect_x_test_rbd, axis=0) == 0:
        ...
        for n in range(np.size(incorrect_x_test_rbd, axis=0)):
            analysed_results_FN = analyzer.analyze(np.reshape(incorrect_x_test_rbd[n, :, :, :, :], (-1, 60, 120, 15, 1)))
            FN[n, :, :, :, :] = analysed_results_FN

        # save numpy_array to .mat file
        np.save('D:/ANT_3D_CNN_source_transfer2/LRP/LRP_FN_sub_%d.npy' % (sub + 1), FN)
