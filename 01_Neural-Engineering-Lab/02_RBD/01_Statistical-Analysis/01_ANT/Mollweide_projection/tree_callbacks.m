                    ...
                    if ~isempty(AllMod)
                        ...
                        for iMod = 1:length(AllMod)
                            ...
                            if ismember(AllMod{iMod}, {'SEEG','ECOG','ECOG+SEEG'}) && ~isempty(sSubject) && ~isempty(DisplayMod) && ismember(AllMod{iMod}, DisplayMod)
                                AddSeparator(jMenuModality);
                                if ~isempty(sSubject.iCortex)
                                    gui_component('MenuItem', jMenuModality, [], 'Display on cortex1', IconLoader.ICON_SURFACE_CORTEX, [], @(h,ev)view_surface_data(sSubject.Surface(sSubject.iCortex).FileName, filenameRelative, AllMod{iMod}));
                                end
                                if ~isempty(sSubject.iAnatomy)
                                    if (length(sSubject.Anatomy) == 1)
                    ...
                    if ~isempty(AllMod)
                        ...
                        for iMod = 1:length(AllMod)
                            ...
                            if ismember(AllMod{iMod}, {'SEEG','ECOG','ECOG+SEEG'}) && ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ~isempty(DisplayMod) && ismember(AllMod{iMod}, DisplayMod)
                                AddSeparator(jMenuModality);
                                gui_component('MenuItem', jMenuModality, [], 'Display on cortex2', IconLoader.ICON_SURFACE_CORTEX, [], @(h,ev)view_surface_data(sSubject.Surface(sSubject.iCortex).FileName, filenameRelative, AllMod{iMod}));
                            end
                        end
                ...
                if (length(bstNodes) == 1)
                    ...
                    if ismember(sStudy.Result(iResult).HeadModelType, {'surface', 'mixed'})
                        if ~isempty(sSubject) && ~isempty(sSubject.iCortex)
                            gui_component('MenuItem', jMenuActivations, [], 'Display on cortex3', IconLoader.ICON_CORTEX, [], @(h,ev)view_surface_data([], filenameRelative));
                        else
                    ...
                    % === DISPLAY ON 2D projection
                    if strcmpi(sStudy.Result(iResult).HeadModelType, 'surface') && ~isempty(sSubject) && ~isempty(sSubject.iCortex)
                        AddSeparator(jMenuActivations);
                        gui_component('MenuItem', jMenuActivations, [], 'Display 2D projection', IconLoader.ICON_SURFACE, [], @(h,ev)view_surface_2dprojection(filenameRelative));
                    end
                end
                ...
                if (length(bstNodes) == 1)
                    ...
                    jMenuActivations = gui_component('Menu', jPopup, [], 'Cortical activations', IconLoader.ICON_RESULTS, [], []);
                        ...
                        if ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ~isVolumeGrid
                            gui_component('MenuItem', jMenuActivations, [], 'Display on cortex4', IconLoader.ICON_CORTEX, [], @(h,ev)view_surface_data([], filenameRelative));
                        end
                        % === DISPLAY ON MRI ===
                        if ~isempty(sSubject) && ~isempty(sSubject.iAnatomy)
                            gui_component('MenuItem', jMenuActivations, [], 'Display on MRI   (3D)', IconLoader.ICON_ANATOMY, [], @(h,ev)view_surface_data(sSubject.Anatomy(sSubject.iAnatomy).FileName, filenameRelative));
                        ...
                        switch lower(DataType)
                            ...
                            case 'results'
                                ...
                                if isempty(strfind(filenameRelative, '_connectn')) && ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ~isVolume
                                    gui_component('MenuItem', jMenuConn1, [], 'Display on cortex5', IconLoader.ICON_CORTEX, [], @(h,ev)view_surface_data([], filenameRelative));
                                end
                                % MRI
                    ...
                    elseif ~isempty(strfind(filenameRelative, '_pac_fullmaps'))
                        ...
                        switch lower(DataType)
                            ...
                            case 'results'
                                ...
                                % Cortex
                                if ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ~isVolume
                                    gui_component('MenuItem', jPopup, [], 'Display on cortex6', IconLoader.ICON_CORTEX, [], @(h,ev)view_surface_data([], filenameRelative));
                                end
                                % MRI
                                if ~isempty(sSubject) && ~isempty(sSubject.iAnatomy)
                                    MriFile = sSubject.Anatomy(sSubject.iAnatomy).FileName;
                    ...
                    elseif ~isempty(strfind(filenameRelative, '_dpac_fullmaps'))
                        ...
                        switch lower(DataType)
                            ...
                            case 'results'
                                ...
                                % Cortex
                                if ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ~isVolume
                                    gui_component('MenuItem', jPopup, [], 'Display on cortex7', IconLoader.ICON_CORTEX, [], @(h,ev)view_surface_data([], filenameRelative));
                                end
                                % MRI
                                if ~isempty(sSubject) && ~isempty(sSubject.iAnatomy)
                                    MriFile = sSubject.Anatomy(sSubject.iAnatomy).FileName;
                    ...
                    % ===== TIME-FREQUENCY =====
                        ...
                        switch lower(DataType)
                                ...
                                if ~isempty(DisplayMod)
                                    ...
                                    for iMod = 1:length(DisplayMod)
                                        ...
                                        elseif ismember(DisplayMod{iMod}, {'SEEG', 'ECOG', 'ECOG+SEEG'}) && ~isempty(sSubject)
                                            if ~isempty(sSubject.iCortex)
                                                gui_component('MenuItem', jMenuModality, [], 'Display on cortex8', IconLoader.ICON_SURFACE_CORTEX, [], @(h,ev)view_surface_data(sSubject.Surface(sSubject.iCortex).FileName, filenameRelative, DisplayMod{iMod}));
                                            end
                                            if ~isempty(sSubject.iAnatomy)
                                                if (length(sSubject.Anatomy) == 1)
                            ...
                            case 'results'
                                ...
                                % Cortex
                                if ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ~isVolume
                                    gui_component('MenuItem', jPopup, [], 'Display on cortex9', IconLoader.ICON_CORTEX, [], @(h,ev)view_surface_data([], filenameRelative));
                                end
                                % MRI
                                if ~isempty(sSubject) && ~isempty(sSubject.iAnatomy)
                                    MriFile = sSubject.Anatomy(sSubject.iAnatomy).FileName;
                ...
                if (length(bstNodes) == 1)
                    ...
                    if strcmpi(DataType, 'data')
                        ...
                        for iMod = 1:length(DisplayMod)
                            ...
                            elseif ismember(DisplayMod{iMod}, {'SEEG', 'ECOG', 'ECOG+SEEG'}) && ~isempty(sSubject)
                                if ~isempty(sSubject.iCortex)
                                    gui_component('MenuItem', jMenuModality, [], 'Display on cortex10', IconLoader.ICON_SURFACE_CORTEX, [], @(h,ev)view_surface_data(sSubject.Surface(sSubject.iCortex).FileName, filenameRelative, DisplayMod{iMod}));
                                end
                                if ~isempty(sSubject.iAnatomy)
                                    if (length(sSubject.Anatomy) == 1)
                                        gui_component('MenuItem', jMenuModality, [], 'Display on MRI (MRI Viewer)', IconLoader.ICON_ANATOMY, [], @(h,ev)view_mri(sSubject.Anatomy(1).FileName, filenameRelative, DisplayMod{iMod}));
                    ...
                    elseif strcmpi(DataType, 'results')                        
                        ...
                        % === DISPLAY ON CORTEX ===
                        if ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ~isVolume
                            gui_component('MenuItem', jMenuActivations, [], 'Display on cortex11', IconLoader.ICON_CORTEX, [], @(h,ev)view_surface_data([], filenameRelative));
                        else
                            gui_component('MenuItem', jMenuActivations, [], 'No cortex available', IconLoader.ICON_WARNING, [], []);
                        end
                        % === DISPLAY ON MRI ===
                        if ~isempty(sSubject) && ~isempty(sSubject.iAnatomy)
                            MriFile = sSubject.Anatomy(sSubject.iAnatomy).FileName;
                            gui_component('MenuItem', jMenuActivations, [], 'Display on MRI   (3D)',         IconLoader.ICON_ANATOMY, [], @(h,ev)view_surface_data(MriFile, filenameRelative));
                            gui_component('MenuItem', jMenuActivations, [], 'Display on MRI   (MRI Viewer)', IconLoader.ICON_ANATOMY, [], @(h,ev)view_mri(MriFile, filenameRelative));
                        else
                            gui_component('MenuItem', jMenuActivations, [], 'No MRI available', IconLoader.ICON_WARNING, [], []);
                        end
                        % === DISPLAY ON SPHERE ==
                        if   ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ismember(sStudy.Timefreq(iTimefreq).DataType, {'results', 'matrix'})
                             AddSeparator(jMenuActivations);
                             gui_component('MenuItem', jMenuActivations, [], 'Display on spheres/squares', IconLoader.ICON_SURFACE, [], @(h,ev)view_surface_sphere(filenameRelative));
                        end    
                        % === DISPLAY ON 2D PROJECTION
                        if   ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ismember(sStudy.Timefreq(iTimefreq).DataType, {'results', 'matrix'})
                            AddSeparator(jMenuActivations);
                            gui_component('MenuItem', jMenuActivations, [], 'Display on 2D projection', IconLoader.ICON_SURFACE, [], @(h,ev)view_surface_2dprojection(filenameRelative));
                        end
                        % MENU: EXPORT (Added later)
