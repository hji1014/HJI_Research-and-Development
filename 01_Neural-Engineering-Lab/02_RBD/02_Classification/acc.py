...
for TL_rbd_num in range(int(max(gt_rbd))):                          # TL : Transfer Learning, PT : Pre-Training
    ...
    history1 = premodel.fit(x_train_PT, y_train_PT, epochs=100, batch_size=128, verbose=2, callbacks=[early_stopping_callback1])
    ...
    print("\n 현재 pre-training 벨리데이션 accuracy :", PT_acc)
    ...
    history2 = transfer_model.fit(x_train_TL, y_train_TL, epochs=100, batch_size=8, verbose=2, callbacks=[early_stopping_callback2])
    ...
    print("\n Tranfer learning 현재 fold 테스트 accuracy :", TL_acc)
...
np.save('D:/all_PT_acc.npy', all_PT_acc)
np.save('D:/all_TL_acc.npy', all_TL_acc)
np.save('D:/all_PT_acc_train.npy', all_PT_acc_train)
np.save('D:/all_TL_acc_train.npy', all_TL_acc_train)
