...
gt_rbd_mci = np.array(gt_rbd_mci, dtype=int)
gt_rbd_nmci = np.sort(gt_rbd_nmci)
gt_rbd_nmci = np.array(gt_rbd_nmci, dtype=int)


# data 가공 #
con_1000 = data1['ANT_con_topo_3d']                                # con/rbd 각 1000 trials
rbd_1000 = data2['ANT_rbd_topo_3d']
rbd_1000_nmci = rbd_1000[gt_rbd_nmci, :, :, :]                           # rbd_nmci
rbd_1000_mci = rbd_1000[gt_rbd_mci, :, :, :]                        # rbd_mci
# mci 수만큼 랜덤추출 (con:13358, nmci:21512, mci:13290)
new_con_index = np.random.choice(int(np.size(con_1000, axis=0)), int(np.size(rbd_1000_mci, axis=0)), replace=False)
new_rbd_nmci_index = np.random.choice(int(np.size(rbd_1000_nmci, axis=0)), int(np.size(rbd_1000_mci, axis=0)), replace=False)
con_1000 = con_1000[new_con_index, :, :, :]
rbd_1000_nmci = rbd_1000_nmci[new_rbd_nmci_index, :, :, :]

con_label = np.zeros(shape=(np.size(con_1000, axis=0), 1))
rbd_label = np.ones(shape=(np.size(rbd_1000_nmci, axis=0), 1))
rbd_mci_label = np.zeros(shape=(np.size(rbd_1000_mci, axis=0), 1))
rbd_mci_label = np.where(rbd_mci_label == 0, 2, rbd_mci_label)
del data1, data2

con_rbd = np.concatenate((con_1000, rbd_1000_nmci, rbd_1000_mci), axis=0)
# 메모리 문제
for s in range(np.size(con_rbd, 0)):
    con_rbd[s, :, :, :] = np.nan_to_num(con_rbd[s, :, :, :])
con_rbd_label = np.concatenate((con_label, rbd_label, rbd_mci_label), axis=0)
del con_1000, rbd_1000, rbd_1000_nmci, rbd_1000_mci, con_label, rbd_label, rbd_mci_label
...
for train, test in skf.split(zz, zzz):
    ...
    #tensorboard = TensorBoard(log_dir="D:/ANT_3D_CNN_con-rbd+nmci-rbd+mci/logs/{}".format(time()))

    history = model.fit(zz[train], zzzz[train], epochs=100, batch_size=128, verbose=2, callbacks=[early_stopping_callback])
    ...
    model.save_weights('D:/ANT_3D_CNN_con-rbd+nmci-rbd+mci/weights/model_weights_fold_%d.h5' % i)
    np.save('D:/ANT_3D_CNN_con-rbd+nmci-rbd+mci/testset/x_fold_%d.npy' % i, zz[test])
    np.save('D:/ANT_3D_CNN_con-rbd+nmci-rbd+mci/testset/y_fold_%d.npy' % i, zzzz[test])
