...
con_balance_idx = np.sort(np.random.choice(np.size(con_1000, axis=0), np.size(rbd_1000, axis=0), replace=False))
con_1000 = con_1000[con_balance_idx, :, :, :]
...
for train, test in skf.split(zz, zzz):
    ...
    tensorboard = TensorBoard(log_dir="D:/ANT_3D_CNN_con-rbd+mci/logs/{}".format(time()))
    ...
    model.save_weights('D:/ANT_3D_CNN_con-rbd+mci/weights/model_weights_fold_%d.h5' % i)
    np.save('D:/ANT_3D_CNN_con-rbd+mci/testset/x_fold_%d.npy' % i, zz[test])
    np.save('D:/ANT_3D_CNN_con-rbd+mci/testset/y_fold_%d.npy' % i, zzzz[test])
