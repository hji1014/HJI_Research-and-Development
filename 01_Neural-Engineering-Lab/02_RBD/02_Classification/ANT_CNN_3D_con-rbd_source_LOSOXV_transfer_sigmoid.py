...
for TL_rbd_num in range(int(max(gt_rbd))):                          # TL : Transfer Learning, PT : Pre-Training
    ...
    premodel = Sequential()
    premodel.add(Conv3D(filters=16, kernel_size=(3, 3, 2), input_shape=(60, 120, 15, 1), activation='relu'))
    ...
    tensorboard1 = TensorBoard(log_dir="D:/ANT_3D_CNN_source_transfer/logs_PT/{}".format(time()))
    ...
    # 모델 weights 저장
    premodel.save_weights('D:/ANT_3D_CNN_source_transfer/premodel_weights/premodel_weights_sub_%d.h5' % sub)
    np.save('D:/ANT_3D_CNN_source_transfer/premodel_testset/premodel_x_sub_%d.npy' % sub, x_val_PT)
    np.save('D:/ANT_3D_CNN_source_transfer/premodel_testset/premodel_y_sub_%d.npy' % sub, y_val_PT)
    ...
    #     i.trainable = False
    # premodel.summary()
    for i in premodel.layers[:16]:      #best 13, 16
        i.trainable = False
    premodel.summary()

    transfer_model = Sequential(premodel.layers[:20])
    ...
    tensorboard2 = TensorBoard(log_dir="D:/ANT_3D_CNN_source_transfer/logs_TL/{}".format(time()))
    ...
    transfer_model.save('D:/ANT_3D_CNN_source_transfer/transfer_model/transfer_model_sub_%d.h5' % sub)
    transfer_model.save_weights('D:/ANT_3D_CNN_source_transfer/transfer_model_weights/transfer_model_weights_sub_%d.h5' % sub)
    np.save('D:/ANT_3D_CNN_source_transfer/transfer_model_testset/transfer_model_x_sub_%d.npy' % sub, x_test_TL)
    np.save('D:/ANT_3D_CNN_source_transfer/transfer_model_testset/transfer_model_y_sub_%d.npy' % sub, y_test_TL)
    # f += 1
