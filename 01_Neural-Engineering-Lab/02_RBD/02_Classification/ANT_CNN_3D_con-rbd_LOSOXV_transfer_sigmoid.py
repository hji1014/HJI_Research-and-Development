...
for TL_rbd_num in range(int(max(gt_rbd))):                          # TL : Transfer Learning, PT : Pre-Training
    ...
    con_TL_num = np.random.choice(int(max(gt_con)), 1, replace=False)
    con_TL_idx = np.where(gt_con == (con_TL_num + 1))
    con_PT_idx = np.where(gt_con != (con_TL_num + 1))
    ...
    con_PT = np.squeeze(con_1000[con_PT_idx, :, :, :])
    con_TL = np.squeeze(con_1000[con_TL_idx, :, :, :])
    con_PT_label = np.reshape(con_label[con_PT_idx, :], (-1, 1))
    con_TL_label = np.reshape(con_label[con_TL_idx, :], (-1, 1))
    ...
    tensorboard1 = TensorBoard(log_dir="logs_PT/{}".format(time()))
    ...
    for train, test in skf.split(con_rbd_TL, con_rbd_TL_label):
        ...
        tensorboard2 = TensorBoard(log_dir="logs_TL/{}".format(time()))

        history2 = transfer_model.fit(con_rbd_TL[train], con_rbd_TL_label[train], epochs=100, batch_size=8, verbose=2, callbacks=[early_stopping_callback2, tensorboard2])

        TL_acc = "%.8f" % (transfer_model.evaluate(con_rbd_TL[test], con_rbd_TL_label[test])[1])
        print("\n Tranfer learning 현재 fold accuracy :", TL_acc)
        all_TL_acc.append(TL_acc)

        # model.save('ANT_CNN_3D_con-rbd_%d.h5' % i)
        # model.save_weights('C:/Users/Nelab_001/PycharmProjects/pythonProject/ANT_CNN_3D_con-rbd_LOSOXV_model_weights/ANT_CNN_3D_con-rbd_%d.h5' % f)
        f += 1

#pre_acc = "%.8f" % (premodel.evaluate(con_rbd_TL[train], con_rbd_TL_label[train])[1])
